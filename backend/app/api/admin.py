...
from ..services.openclaw_runtime import resolve_data_dir
...
async def deep_health(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        pong = await cast(Any, bus.redis).ping()
...
async def migration_check():
    ...
    files = {
        "artifacts_dir": (data_dir / "artifacts").exists(),
        "notify_config": (data_dir / "notify_config.json").exists(),
        "morning_brief": (data_dir / "morning_brief.json").exists(),
