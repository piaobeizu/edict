...
GROUP = "orchestrator"
CONSUMER = f"orch-{os.getpid()}-{uuid.uuid4().hex[:6]}"
...
class OrchestratorWorker:
            ...
            if events:
                ...
                for entry_id, event in events:
                    try:
                        ...
                        log.error(
                            f"Error recovering event {entry_id} from {topic}: {e}",
                            exc_info=True,
                        )
    ...
    async def _clear_dedup(self, event: dict):
        """回滚去重标记，允许事件被重新处理。"""
        event_id = event.get("event_id")
        if event_id:
            key = f"edict:orch:event:{event_id}"
    ...
    async def _on_task_created(self, payload: dict, trace_id: str):
        ...
        try:
            state_enum = TaskState(state_str)
        except ValueError:
            state_enum = TaskState.Taizi
        agent = STATE_AGENT_MAP.get(state_enum, "taizi")
