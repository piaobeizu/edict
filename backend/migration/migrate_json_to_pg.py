...
def _map_priority(value: str | None) -> str:
    ...
    if text in {"高", "high", "urgent", "p0", "p1"}:
        return "high"
    if text in {"低", "low", "p4"}:
        return "low"
    return "normal"
...
def _normalize_task_id(raw: str | None) -> str:
    task_id = (raw or "").strip()
    if task_id:
        return task_id[:32]
    return f"MIG-{uuid.uuid4().hex[:28]}"
...
def parse_old_task(old: dict) -> dict:
    ...
    legacy_id = _normalize_task_id(old.get("id", ""))
    ...
    return {
        ...
        "title": title,
        "state": state.value,
        "org": old.get("org") or "太子",
        "official": old.get("official", "emperor"),
        "now": old.get("now", ""),
        "eta": old.get("eta", "-"),
        "block": old.get("block", "无"),
        "output": old.get("output", ""),
        "priority": _map_priority(old.get("priority")),
        ...
        "template_id": old.get("template_id") or old.get("templateId", ""),
        "template_params": old.get("template_params") or old.get("templateParams") or {},
        "ac": old.get("ac", ""),
        "target_dept": old.get("target_dept") or old.get("targetDept", ""),
...
async def migrate(file_path: Path, dry_run: bool = False):
    ...
    if dry_run:
        ...
        for old in old_tasks:
            ...
            log.info(f"  [{params['id']}] {params['title'][:40]} → {params['state']}")
        ...
        for old in old_tasks:
            try:
                ...
                existing = await db.execute(
                    select(Task).where(Task.id == legacy_id)
                )
