...
def handle_review_action(task_id, action, comment=""):
    """门下省/御览审批：准奏/封驳。支持 Menxia、YuLan、Review 三种状态。"""
    tasks = load_tasks()
    task = next((t for t in tasks if t.get("id") == task_id), None)
    if not task:
        return {"ok": False, "error": f"任务 {task_id} 不存在"}
    if task.get("state") not in ("Review", "Menxia", "YuLan"):
        return {
            "ok": False,
    ...
    if action == "approve":
        if task["state"] == "Menxia":
            # 门下省准奏 → 进入御览（等待皇上审批）
            task["state"] = "YuLan"
            task["now"] = "门下省准奏，呈送皇上御览"
            remark = f"✅ 准奏：{comment or '门下省审议通过，呈送御览'}"
            to_dept = "皇上"
        elif task["state"] == "YuLan":
            # 皇上御批准奏 → 转尚书省派发
            task["state"] = "Assigned"
            task["now"] = "皇上御批准奏，移交尚书省派发"
            remark = f"👑 御批准奏：{comment or '皇上准奏'}"
            to_dept = "尚书省"
        else:  # Review
            task["state"] = "Done"
            task["now"] = "御批通过，任务完成"
    ...
    # 🚀 审批后自动派发对应 Agent（YuLan 状态不自动派发，等待人工）
    new_state = task["state"]
    if new_state not in ("Done", "YuLan"):
    ...
    if new_state == "YuLan":
        ...
        dispatched = " (已自动派发 Agent)" if new_state not in ("Done", "YuLan") else ""
...
def get_agents_status():
    ...
    gateway_alive = _check_gateway_alive() or gateway_probe
...
def handle_scheduler_retry(task_id, reason=""):
    ...
    if not task:
        return {"ok": False, "error": f"任务 {task_id} 不存在"}
    state = task.get("state", "")
    if state in _TERMINAL_STATES or state in ("Blocked", "YuLan"):
...
_STATE_FLOW = {
    ...
    "Menxia": ("YuLan", "门下省", "皇上", "门下省准奏，呈送皇上御览"),
    "YuLan": ("Assigned", "皇上", "尚书省", "皇上御批准奏，转尚书省派发"),
...
def dispatch_for_state(task_id, task, new_state, trigger="state-transition"):
    ...
    def _do_dispatch():
        try:
            if not (_check_gateway_alive() or _check_gateway_probe()):
            ...
            _update_task_scheduler(
                ...
                lambda t, s: (
                    ...
                    _scheduler_add_flow(
                        t, f"派发失败：{agent_id}（{trigger}）", to=t.get("org", "")
                    ),
                ),
            )
        except subprocess.TimeoutExpired:
