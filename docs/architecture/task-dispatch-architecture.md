    ...
    if role == 'assistant':
        entry = {
            ...
            'tools': [
                ...
                {'name': 'file_read', 'input_preview': 'backend/app/api/dashboard.py'},
            ]
        }
        return entry
...
响应：
{
  ...
  "activity": [
    ...
    // - assistant (10条)
    {
      ...
      "thinking": "💭 考虑到项目的复杂性，需要覆盖七个Agent的交互逻辑。单元测试应该采用pytest，集成测试用 compose 启动后的 backend API HTTP 测试...",
      "tools": [
        {"name": "bash", "input_preview": "find . -name '*.py' -type f | wc -l"},
        {"name": "file_read", "input_preview": "backend/app/api/dashboard.py (first 100 lines)"}
      ]
    },
    // - tool_result (16条)
    {
...
```bash
python3 backend/app/runtime_assets/kanban_update.py create \
  JJC-20260228-E2E \
  "为三省六部编写完整自动化测试方案" \
  Zhongshu \
  中书省 \
...
```bash
python3 backend/app/runtime_assets/kanban_update.py state \
  JJC-20260228-E2E \
  Menxia \
  "方案提交门下省审议"
...
```bash
python3 backend/app/runtime_assets/kanban_update.py flow \
  JJC-20260228-E2E \
  "中书省" \
  "门下省" \
  "📋 方案提交审核，请审议"
...
```bash
python3 backend/app/runtime_assets/kanban_update.py progress \
  JJC-20260228-E2E \
...
```bash
python3 backend/app/runtime_assets/kanban_update.py done \
  JJC-20260228-E2E \
...
```bash
# 叫停（随时可恢复）
python3 backend/app/runtime_assets/kanban_update.py stop \
  JJC-20260228-E2E \
  "等待工部反馈继续"
...
#
# 恢复：
python3 backend/app/runtime_assets/kanban_update.py resume \
  JJC-20260228-E2E \
  "工部已反馈，继续执行"
#
...
# 取消（不可恢复）
python3 backend/app/runtime_assets/kanban_update.py cancel \
  JJC-20260228-E2E \
  "业务需求变更，任务作废"
#
...
> 说明：本节面板清单用于阐释架构设计与观测维度，可能与 README 中当前版本的 UI 面板命名/分组存在细微差异；以 README 与实际界面为准。
...
curl -X POST http://127.0.0.1:8001/api/create-task \
...
curl http://127.0.0.1:8001/api/task-activity/JJC-20260302-001
