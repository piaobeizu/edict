...
> `scripts/skill_manager.py` 当前版本尚未落地实现。以下命令仅为规划中的 CLI 形态，请优先使用上面的 API 端点或看板 UI。
...
详见 [任务分发流转架构文档](../architecture/task-dispatch-architecture.md) 的第三部分（API 与工具）。
