...
> 说明：`scripts/skill_manager.py` CLI 仍在规划中，当前请优先走 **看板 UI** 或 **HTTP API**。下文 CLI 示例仅用于展示未来命令形态。
...
# 前端: http://127.0.0.1:8002  后端 API: http://127.0.0.1:8001
```

### 2. 添加官方 Skill（规划中的 CLI 形态）
...
```bash
curl http://127.0.0.1:8001/api/remote-skills-list | jq .
...
### 一键导入官方库中的所有 skills（规划中的 CLI 形态）
...
```bash
curl -X POST http://127.0.0.1:8001/api/update-remote-skill \
  -H "Content-Type: application/json" \
  -d '{
    "agentId": "zhongshu",
    "skillName": "code_review"
...
```bash
curl -X POST http://127.0.0.1:8001/api/remove-remote-skill \
  -H "Content-Type: application/json" \
  -d '{
    "agentId": "zhongshu",
    "skillName": "code_review"
...
```bash
# 检查 API 是否可返回远程技能列表
curl -s http://127.0.0.1:8001/api/remote-skills-list | jq .
...
- 🏛️ [架构文档](../architecture/task-dispatch-architecture.md)
