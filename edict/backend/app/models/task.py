...
STATE_TRANSITIONS = {
    ...
    TaskState.Menxia: {TaskState.YuLan, TaskState.Zhongshu, TaskState.Cancelled},  # 封驳退回中书
    TaskState.YuLan: {TaskState.Assigned, TaskState.Zhongshu, TaskState.Cancelled, TaskState.Blocked},
...
class Task(Base):
    ...
    id = Column("task_id", String(32), primary_key=True, comment="任务ID, e.g. JJC-20260301-001")
    title = Column(Text, nullable=False, comment="任务标题")
    state = Column(String(20), nullable=False, default=TaskState.Taizi.value, index=True)
    ...
    def to_dict(self) -> dict:
        ...
        todos = self.todos or []
        # Sprint2 一致性兜底：Done/Cancelled 任务若无 todos，生成完成版骨架，避免 UI 漂移
        if not todos and self.state in {TaskState.Done, TaskState.Cancelled}:
            todos = [
                {"id": 1, "title": "分析旨意", "status": "completed"},
                {"id": 2, "title": "起草方案", "status": "completed"},
                {"id": 3, "title": "门下审议", "status": "completed"},
                {"id": 4, "title": "皇上御览", "status": "completed"},
                {"id": 5, "title": "尚书执行", "status": "completed"},
                {"id": 6, "title": "回奏皇上", "status": "completed"},
            ]

        return {
            "id": self.id,
            "title": self.title,
            "state": self.state.value if isinstance(self.state, TaskState) else (self.state or ""),
            "org": self.org,
