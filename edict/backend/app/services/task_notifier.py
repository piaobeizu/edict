...
复用 dashboard/notify 推送系统，在 Worker 中以 best-effort 方式调用。
推送失败不影响主流程。
"""
...
def _get_notify_service():
    ...
    try:
        # 确保 notify 包可导入
        for candidate in [Path("/app"), Path("/app/dashboard")]:
            if (candidate / "notify").is_dir() and str(candidate) not in sys.path:
                sys.path.insert(0, str(candidate))
                break
        # 本地开发 fallback
        dashboard_dir = Path(__file__).resolve().parents[4] / "dashboard"
        if dashboard_dir.exists() and str(dashboard_dir) not in sys.path:
