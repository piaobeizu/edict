...
export default function TaskModal() {
  ...
  // Agent 正在执行中 → 禁用准奏/封驳/推进，仅保留叫停和取消
  const isDispatching = schedData?.scheduler?.lastDispatchStatus === 'running';
        ...
        <div className="modal-body">
          ...
          <div className="task-actions">
            ...
            {isDispatching && (
              <div style={{ width: '100%', padding: '8px 12px', background: '#3b82f615', border: '1px solid #3b82f633', borderRadius: 8, marginBottom: 8, fontSize: 12, color: '#60a5fa', display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                Agent 正在执行中，请等待完成后再操作
              </div>
            )}
            {task.state === 'YuLan' && (
              <div style={{ width: '100%', padding: '12px', background: '#ffd70015', border: '1px solid #ffd70044', borderRadius: 8, marginBottom: 8, opacity: isDispatching ? 0.4 : 1 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button className="btn-action" disabled={isDispatching} style={{ background: '#2ecc8a22', color: '#2ecc8a', border: '1px solid #2ecc8a44', flex: 1, fontWeight: 700, fontSize: 14, padding: '8px 0', cursor: isDispatching ? 'not-allowed' : 'pointer' }} onClick={() => doReview('approve')}>👑 准奏 — 交付执行</button>
                  <button className="btn-action" disabled={isDispatching} style={{ background: '#ff527022', color: '#ff5270', border: '1px solid #ff527044', flex: 1, fontWeight: 700, fontSize: 14, padding: '8px 0', cursor: isDispatching ? 'not-allowed' : 'pointer' }} onClick={() => doReview('reject')}>🚫 封驳 — 退回修改</button>
                </div>
              </div>
            )}
          ...
          <div className="sched-section">
            ...
            <div className="sched-actions">
              <button className="sched-btn" disabled={isDispatching} onClick={() => doSchedAction('retry')}>🔁 重试派发</button>
              <button className="sched-btn warn" disabled={isDispatching} onClick={() => doSchedAction('escalate')}>📣 升级协调</button>
              <button className="sched-btn danger" disabled={isDispatching} onClick={() => doSchedAction('rollback')}>↩️ 回滚稳定点</button>
